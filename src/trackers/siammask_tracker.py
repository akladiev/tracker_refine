...
from src.utils.bbox import cxy_wh_2_rect
...
class SiamMaskTracker(SiamRPNTracker):
    def __init__(self, model):
        ...
        assert hasattr(self.net, 'mask_head'), \
            "SiamMaskTracker must have mask_head"
        assert hasattr(self.net, 'refine_head'), \
    ...
    def track(self, img):
        ...
        x_crop = self.get_subwindow(
            img, self.center_pos, cfg.TRACK.INSTANCE_SIZE, s_x, self.channel_average
        )
        ...
        outputs = self.net.track(xf, self.zf)
        ...
        mask = self.net.mask_refine((delta_y, delta_x)).sigmoid().squeeze()
        ...
        if cfg.REFINE_TEMPLATE.METHOD != 'OFF':
            ...
            outputs['zf'] = current  # for dataset collection
