...
class VOTVideo(Video):
    ...
    def load_tracker(self, path, tracker_names=None, store=True):
        ...
        if not tracker_names:
            tracker_names = [Path(x).stem for x in glob(path) if Path(x).is_dir()]
        if not isinstance(tracker_names, list):
            tracker_names = [tracker_names]
        for name in tracker_names:
            traj_files = glob(os.path.join(path, name, f'{self.name}.txt')) + \
                         glob(os.path.join(path, name, self.name, '*.txt'))
    ...
    def __init__(self, name, dataset_root, load_img=False):
        ...
        for video in pbar:
            ...
            self.videos[video] = VOTVideo(video,
                                          ...
                                          meta_data[video].get('camera_motion', ''),
                                          meta_data[video].get('illum_change', ''),
                                          meta_data[video].get('motion_change', ''),
                                          meta_data[video].get('size_change', ''),
                                          meta_data[video].get('occlusion', ''),
                                          load_img=load_img)
